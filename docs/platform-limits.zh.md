...
| 产品 | 免费版 | 付费版 | Scope | 备注 |
| --- | --- | --- | --- | --- |
| Cloudflare Workers | 10 万次请求/天;单次请求 CPU 时间最多 10 ms | Workers Paid 起步 $5/月, 含 1000 万次请求/月;CPU 限制更高(默认 30s, HTTP 请求可配到 300s) | 按账号 | 适合作为通用边缘函数运行平台 |
...
### Cloudflare D1
- 单数据库最大: 500 MB(Free) / 10 GB(Workers Paid)
